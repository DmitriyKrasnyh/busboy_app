...
import { Table, MenuItem, Order, OrderItem, HallLayout } from '../types';
...
type RestaurantAction =
  | { type: 'SET_TABLE_STATUS'; tableId: number; status: Table['status']; guests?: number; waiterId?: number }
  | { type: 'ADD_ORDER_ITEM'; tableId: number; item: OrderItem; waiterId: number }
  | { type: 'REMOVE_ORDER_ITEM'; tableId: number; itemIndex: number }
  | { type: 'COMPLETE_ORDER'; tableId: number; waiterId: number }
  ...
  | { type: 'UPDATE_HALL_LAYOUT'; layout: Partial<HallLayout> }
        ...
        tables: state.tables.map(table =>
          table.id === action.tableId
            ? {
                ...
                waiterId: action.waiterId || table.waiterId,
    ...
    if (savedData) {
      try {
        ...
        const restoredData = {
          ...
          orders: parsedData.orders.map((order: any) => ({
            ...order,
            timestamp: new Date(order.timestamp)
          }))
        };
